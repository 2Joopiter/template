...
import { fetchDepartment, fetchHistory, fetchYoutube, fetchFlickr, fetchDepartInfo } from './api';
...
function* callDepartInfo() {
	yield takeLatest(types.DEPARTINFO.start, function* () {
		try {
			const response = yield call(fetchDepartInfo);
			yield put({ type: types.DEPARTINFO.success, payload: response.about });
		} catch (err) {
			yield put({ type: types.DEPARTINFO.fail, payload: err });
		}
	});
}
...
export default function* rootSaga() {
	yield all([fork(callMembers), fork(callHistory), fork(callDepartInfo), fork(callYoutube), fork(callFlickr)]);
}
